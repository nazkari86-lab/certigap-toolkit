...
| Native TrackingAutoIndex removes Python hot-path overhead | Differential and empirical | sanitizer/randomized cross-language tests and `results/tracking_autoindex_native_runtime.csv` | Matching deterministic streams on one machine; `113.2x-13053.2x` faster than Python uniform tracking, but `13.1x` median slower than the fastest fixed C++ backend |
| Rebuild-aware native migration prevents uniform-cost thrashing on measured read-mostly shifts | Empirical ablation | `results/tracking_autoindex_native_runtime.csv` | `n=256,4096` deterministic streams; `360x-394x` fewer switches and `3.5x-11.5x` lower runtime, not a universal workload guarantee |
