...
Status: CertiGap Toolkit `v1.12.0`.
...
| Native TrackingAutoIndex removes Python hot-path overhead | Differential and empirical | sanitizer/randomized cross-language tests and `results/tracking_autoindex_native_runtime.csv` | Matching deterministic streams on one machine; `106.0x-15919.5x` faster than Python uniform tracking, but `10.9x` median slower than the fastest fixed C++ backend |
| Rebuild-aware native migration prevents uniform-cost thrashing on measured read-mostly shifts | Empirical ablation | `results/tracking_autoindex_native_runtime.csv` | `n=256,4096` deterministic streams; `360x-394x` fewer switches and `3.6x-14.8x` lower runtime, not a universal workload guarantee |
| FastTrackingAutoIndex bounds measured controller overhead to a few times a robust dynamic baseline | Randomized differential and empirical | 48,000-operation ASan/UBSan validation and `results/tracking_autoindex_fast_runtime.csv` | 64 deterministic sum configurations on this machine: `1.82x` median, `3.58x` p95, `3.89x` maximum versus Fenwick; no universal latency theorem |
| FastTrackingAutoIndex does not match the best specialized backend chosen with future knowledge | Empirical negative result | `results/tracking_autoindex_fast_runtime.csv` | `2.08x` median and `8.16x` maximum versus fastest fixed hindsight; that comparator includes update-only arrays and query-only prefix sums |
...
| `adaptive_array` automatically profiles and applies a fail-closed score gate | Native behavioral and persistence validation | `results/adaptive_array_validation.csv` and C++ compile/run tests | Complete eight-candidate structural model, zero-based API, synchronous maintenance, and strict workload-profile persistence; not a statistical or wall-clock no-regression guarantee |
...
| Measured deployment fails closed against a conventional baseline | Statistical decision replay and runtime differential | `results/measured_deployment_validation.csv`, measured replay tests, and `verify_measured_deployment_artifact` | One-sided Hoeffding bound for bounded normalized paired batch harm under representative independent repetitions; not p99, future-drift, cross-machine, or production-SLO certification |
