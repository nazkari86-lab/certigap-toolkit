...
| Native TrackingAutoIndex removes Python hot-path overhead | Differential and empirical | sanitizer/randomized cross-language tests and `results/tracking_autoindex_native_runtime.csv` | Matching deterministic streams on one machine; `173.2x-15952.7x` faster than Python uniform tracking, but `11.2x` median slower than the fastest fixed C++ backend |
| Rebuild-aware native migration prevents uniform-cost thrashing on measured read-mostly shifts | Empirical ablation | `results/tracking_autoindex_native_runtime.csv` | `n=256,4096` deterministic streams; `360x-394x` fewer switches and `3.6x-16.4x` lower runtime, not a universal workload guarantee |
