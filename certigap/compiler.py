...
def _selected_topology(
    ...
    name = selected_name or artifact["selected"]
    selected = next(
        row for row in artifact["candidates"]
        if row["name"] == name
    )
    if not name.startswith("certirange"):
        return []
...
def generate_cpp_header(
    ...
    *,
    ...
    selected = selected_name or artifact["selected"]
    if selected not in _BACKEND_ENUM:
        raise CompileInputError("selected backend is unknown")
    selected_row = next(
        row for row in artifact["candidates"] if row["name"] == selected
    )
    if not selected_row["feasible"]:
        raise CompileInputError("selected backend is infeasible")
    rows = _selected_topology(artifact, selected)
    aggregate = artifact["constraints"]["aggregate"]
    digest = deployment_sha256 or artifact["sha256"]
    if not isinstance(digest, str) or not re.fullmatch(r"[0-9a-f]{64}", digest):
        raise CompileInputError("deployment digest must be lowercase SHA-256")
    scope = selection_scope or (
        "complete deterministic eight-candidate training portfolio"
    )
    if not isinstance(scope, str) or not scope or '"' in scope:
        raise CompileInputError("selection scope cannot be emitted safely")
    lines = [
    ...
    lines.extend(
        [
            ...
            (
                ...
                f"{selected_row['train']['score']:.17g};"
            ),
            "",
        ]
    )
