...
from .safe_autoindex import SafeSelectionPolicy, compile_safe_autoindex
from .safe_autoindex_verifier import verify_safe_autoindex_certificate


SAFE_INPUT_SCHEMA = "certigap-safe-compile-input-v1"


def load_safe_compile_spec(
    raw: object,
) -> tuple[
    list[float],
    WorkloadTrace,
    ...
    if not isinstance(raw, dict):
        raise CompileInputError("safe compile input must be a JSON object")
    allowed = {
        "schema",
        "values",
        "train_trace",
    ...
    if raw.get("schema") != SAFE_INPUT_SCHEMA:
        raise CompileInputError(f"schema must be {SAFE_INPUT_SCHEMA}")
    ...
    try:
        ...
        policy = SafeSelectionPolicy(**policy_fields)
        policy.validate()
    except (TypeError, ValueError) as exc:
        raise CompileInputError(
            f"safe constraints or policy do not validate: {exc}"
        ) from exc
    return values, train, validation, test, constraints, policy


def compile_safe_spec(raw: object) -> dict:
    values, train, validation, test, constraints, policy = (
        load_safe_compile_spec(raw)
    )
    return compile_safe_autoindex(
        values,
        train,
        validation,
        test_trace=test,
